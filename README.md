...
避免每次叶子求值重新扫描整盘。
模型格式现为 v12，策略与搜索覆盖棋盘上全部合法空点。局部轴棋形同时使用精确残差
查表和可共享的射线分解表示；推理前会把两者烘焙为单张轴棋形表。Policy 通过 Rank-4
低秩双线性交互按全局局势调制局部棋形，Value 使用均值、最大值和归一化指数平滑
最大值聚合全部候选威胁，既保留极强单点信号，也让多个强威胁获得平滑梯度。价值塔采用适合
...
开始全新 v12 训练时，应先停止旧进程，再只删除该实验对应文件和旧配置：
