...
默认每个训练周期生成至少 50,000 个新局面；黑方每步搜索 400 次，白方每步搜索
800 次。Collector 只按
...
模型与进度保存。
自博弈保持空棋盘起步，但使用近期冠军联赛：70% 为当前冠军左右互搏，20% 为当前
冠军执白对随机历史冠军执黑，10% 为当前冠军与同一历史冠军交换颜色的成对对局。
历史方搜索仍参与完整对局和统计，但只有当前冠军控制的着法写入 Replay，避免用旧模型
策略监督新模型。历史池为空时自动回退为当前冠军自博弈。Arena 的成对随机开局保持独立。
...
分别不低于45%，再通过历史冠军门槛后才覆盖 `best.safetensors`。按 Ctrl+C 会在安全边界保存并退出。
周期性 Arena 默认先生成 2 ply 随机开局，再由在线候选与 Best 接管；相邻两盘复用
同一开局并交换黑白。在线候选通过总下界、分颜色和历史冠军三重门槛后才覆盖
`best.safetensors`，并同时更新 Actor 的当前冠军与历史对手池。
已有 v25 训练可无损续跑：将配置的 `format_version` 改为 18，并加入
`selfplay_white_simulations = 800`、`selfplay_current_fraction = 0.7`、
`selfplay_current_white_history_fraction = 0.2`、`selfplay_paired_history_fraction = 0.1`
和 `arena_color_score_floor = 0.45`；模型、Replay、进度与历史冠军均无需删除。
...
模型格式为 v25。Policy 使用 16 维轻量全局上下文动态生成局部棋形打分权重，再与
...
开始全新训练配置 v18 及模型格式 v25 时，应先停止旧进程，再只删除该实验对应文件和旧配置。
