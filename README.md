...
手工战术特征；Policy 的位置 bias 也从全零开始训练。Value 只使用完整对局的最终
胜、和、负作为 WDL 监督，不混入当前网络搜索产生的 Q 值。
MCTS 节点还缓存黑白双视角的增量累加器，扩展子节点时只加入新落子和手数特征，
避免每次叶子求值重新扫描整盘。

模型格式现为 v9，价值塔采用适合 NEON/AVX2/FMA 点积的输出优先布局。旧模型不再
...
开始全新 v9 训练时，应先停止旧进程，再只删除该实验对应文件和旧配置：
