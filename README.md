...
v32 使用独立模型、回放和进度路径，需要重新开始训练；样本 JSON 表示中的终局结果字段为必填 `mc_value`。
...
第一次 Ctrl+C 请求停止：先原子保存当前已发布模型及对应进度，再等待工作线程结束并保存回放。第二次 Ctrl+C 请求跳过或取消回放保存，不取消模型保存。保存回放前立即打印提示，进行中每秒报告样本数量和历史节点数量，完成后报告大小及耗时。

回放采用 `GO19RP01` 二进制 + LZ4 流式快照，棋盘历史按父节点编号去重，共享历史只写一次。读取时重建共享结构，保留超级劫、对称视角和所有样本权重。临时文件完整写入后才原子替换，取消不破坏已有完整快照。模型与回放都成功读取后才消费续训文件，旧格式或损坏的回放不会提前删除模型。

**回放格式已升级，不兼容旧 JSONL/LZ4 回放。** 保留旧文件，将已有配置改为新的路径，例如：

```toml
replay_path = "data/go19-v32/replay.bin.lz4"
```

模型格式仍为 v32，无需重练；新路径首次从空回放池重新收集。默认配置使用上述路径。已运行的旧进程不会自动切换保存逻辑，需重新编译后生效。

验证：66 项测试通过，包含共享历史、全部对称样本、回放往返、截断文件与取消写入。实际控制台验证第一次 Ctrl+C 保存模型和回放并成功恢复，以及连续两次 Ctrl+C 保存模型后跳过回放。扩展测试池 81920 条、701 个唯一历史节点，快照约 15 MB，保存 0.39 秒、恢复 0.18 秒；这是大量共享数据的本地测试，不代表真实 50 万条回放的固定耗时。
