...
训练设备更新。macOS 优先使用 Metal，其他环境在加速设备不可用时回退 CPU；批大小由配置文件控制。
...
避免每次叶子求值重新扫描整盘。
搜索同时维护 `Unknown/Win/Draw/Loss` 的已证明状态：终局规则产生的确定结果沿树递归
传播，根节点还精确验证自己一步成五和会放任对手下一步成五的着法；已证明结果优先于
概率 Value。模拟预算足以覆盖根节点全部合法着时，每个未证明根着至少检查一次；
极低预算仍从最高先验开始，不引入坐标顺序偏置。自博弈遇到已证明胜着时直接生成
确定性策略标签，不伪造访问次数。

模型格式现为 v10，价值塔采用适合 NEON/AVX2/FMA 点积的输出优先布局。旧模型不再
