...
CPU 推理预先维护行、列和两类对角线的棋子索引，两个 Attention Head 在一次邻接
遍历中同时计算；225 个位置的轴线编号与同线位图在编译期生成。固定尺寸投影在
AArch64 使用 NEON，在 x86-64 使用 AVX2/FMA。双视角逐层增量 K/V 缓存已有等价性
实现和测试，但当前两层网络的受影响集合扩散与缓存复制成本高于完整稀疏前向，因此
MCTS 默认不启用该缓存。
...
设备；CUDA 初始化失败会直接报错，不再静默退回 CPU。一个训练 batch 会按其中最大
Token 数和合法着点数填充后一次送入 GPU，不再逐样本构建计算图；有效 Token 仍保持
变长语义，同行、同列和双对角线掩码与 CPU 推理一致。棋子/位置嵌入、两层 Q/K/V、Attention
