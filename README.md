...
默认每个训练周期收集 1,000 盘自博弈，每步搜索 3,000 次；每次更新依次执行并行
自博弈、回放池裁剪、策略价值训练、模型与进度保存。
...
邻域密度等 24 个战术特征，使相同局部攻防形状能够跨位置迁移。Value 只使用完整
对局的最终胜、和、负作为 WDL 监督，不混入当前网络搜索产生的 Q 值。
Value 塔额外接收 12 维全局战术汇总，包括双方立即胜点、四威胁点、开放三点、
双四/双三点数量和最大威胁强度；推理时直接复用 Policy 的候选点扫描结果。
MCTS 节点还缓存黑白双视角的增量累加器，扩展子节点时只加入新落子和手数特征，
避免每次叶子求值重新扫描整盘。

模型格式现为 v8，价值塔采用适合 NEON/AVX2/FMA 点积的输出优先布局。旧模型不再
...
开始全新 v8 训练时，应先停止旧进程，再只删除该实验对应文件：
