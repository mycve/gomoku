...
模型与进度保存。
80% 自博弈默认使用模型原始策略生成高温开局：每手以 `1/6` 概率结束、最多 18 手，
期望长度接近 6 手；策略温度为 1.6，随后交给 MCTS。这些开局手不写入训练样本，
其余 20% 从标准空棋盘开始搜索。可通过 `selfplay_policy_opening_probability` 调整或
完全关闭。Arena 的成对随机开局逻辑保持独立。
...
避免每次叶子求值重新扫描整盘。
模型格式为 v16。搜索、训练与模型策略均覆盖规则允许的全部空点，不再使用空间半径
裁剪；立即成五、必须阻五和根节点开放四仍会将搜索收缩到强制战术着。根节点
Dirichlet 噪声使用固定总浓度 10.83，并按当时合法着数量动态换算单动作 alpha。
搜索使用随访问量对数增长的动态 PUCT、DAG 转置复用和基于价值标准误差的 LCB 落子
排序；根节点随机取 2 种棋盘对称平均网络输出，并使用独立于树内策略的温度调度。
...
开始全新训练配置及 Replay v5 时，应先停止旧进程，再只删除该实验对应文件和旧配置：
