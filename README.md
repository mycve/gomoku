...
默认每个训练周期生成至少 50,000 个新局面，每步搜索 400 次；Collector 只按
...
模型与进度保存。
99% 自博弈先生成 KataGomo 风格的平衡开局：按加权分布生成 0–9 手邻近随机棋形，
再穷举合法非终局下一手，按 `(1-v²)^4` 抽取价值更接近均势的着法。20 次严格重试仍失败后，
将拒绝率从 0.995 降为 0.8 并继续寻找，不强制接受坏开局。之后默认所有对局执行温度 1.6 的 Policy Init：
手数按均值 6 的指数分布采样，再扣除 `2 × 已有开局手数`；每手有 0.02% 概率在合法着中均匀抽取。
所有开局手都不写入训练样本。
可分别通过 `selfplay_balanced_opening_probability` 和 `selfplay_policy_opening_probability` 调整。Arena 的成对随机开局保持独立。
...
避免每次叶子求值重新扫描整盘。
模型格式为 v19。Policy 使用共享的 64 维全局与局部逐点门控融合层，经 ReLU 后生成全盘 logits，
保留完整的全局局势与局部棋形非线性交互。搜索、训练与模型策略均覆盖规则允许的全部空点，不再使用空间半径
